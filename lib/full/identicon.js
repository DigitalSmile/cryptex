...
function generateIdenticons(element) {
    "use strict";
    ...
    // hash extraction pattern
        ...
        // For each <img> with class=identicon build canvas identicon
        //Array.prototype.slice.call(document.getElementsByClassName('identicon')).forEach(canvas_idention);
        canvas_idention(element);
    function canvas_idention(self) {
        // Extract hash from the DOM
        var hash = $(self).attr('data-identicons');
        ...
        $(self).removeAttr('data-identicons');
        ...
        draw_block(s[3], s[4], s[5], 0, 0); // A block
        draw_block(s[6], s[7], s[8], 0, 1); // B blocks
        draw_block(s[6], s[7], s[8], 1, 0);
        draw_block(s[9], s[10], 0, 1, 1); // C block - Rotationally symmetric blocks 0-7 only
        ...
        $(self).attr('src',c.toDataURL());

        // ----- End -----


        // Block drawing function
        function draw_block(type, invert, angle, x, y) {
            cx.save();
